...
<body>
    <form id="frmMovie" method="POST" enctype="multipart/form-data" action="insertproses.php">
        ...
        <div class="form-group">
            <table id="daftarpemain">
                <tr>
                    <th>ID</th><th>Nama</th><th>Peran</th><th>Action</th>
                </tr>
            </table>
        </div>
        <div class="form-group">
            <input type="submit" name="submit" id="submit" value="Tambah Movie">
        </div>
    </form>
    <script src="jquery-3.5.1.min.js"></script>
    <script>

        $("#frmMovie").submit(function(event) {
            event.preventDefault();

            var formData = new FormData($("#frmMovie")[0]);
            
            $.ajax({
                url: 'insertproses.php',
                type: 'POST',
                data: formData,
                async: false,
                cache: false,
